...
// Log startup mode
const mode =
  process.env.DEPLOYMENT === "vercel" || process.env.VERCEL
    ? "production"
    : "local";
...
async function handleConfig(
  ...
  if (!isLocalhost(req, server)) {
    if (!configUpdate.auth) {
      return Response.json(
        { error: "Auth required for non-localhost requests" },
        { status: 401 },
      );
    }
...
/**
 * Wait for all inflight tasks to complete, then emit worker.drained
 */
async function waitForDrained(): Promise<void> {
  const pollInterval = 50; // ms
  const maxWait = config.drainBufferMs - 100; // Leave 100ms buffer for exit
  const startTime = Date.now();
...
async function handleDrain(
  req: Request,
  serverInterface: { requestIP(req: Request): { address: string } | null },
): Promise<Response> {
  // Parse optional JSON body for auth
  let body: {
    auth?: { token: string; issued_at: number; ttl: number };
    worker_id?: string;
  } = {};
  try {
  ...
  // Emit worker.draining immediately
  console.log(
    JSON.stringify({ event: "worker.draining", workerId: worker.workerId }),
  );

  // Start drain process (non-blocking) - wait for inflight, emit drained, exit
  waitForDrained().then(() => process.exit(0));
...
function handleShutdown() {
  ...
  // Wait for inflight tasks to complete, emit drained, then exit
  waitForDrained().then(() => {
    server.stop();
    process.exit(0);
  });
